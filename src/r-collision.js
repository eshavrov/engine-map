...
var rectCollision = {
	...
	addRect: function (i, x, y, w, h) {
			this.list.push({
							i: i, visibled: true, rect: {x: x << 0, y: y << 0, w: w << 0, h: h << 0}}
			);
	},
					...
					for (var i = 0; i < this.list.length; i++) {
							...
							if (_tmp === true && this.listBoundRect.length > 0) {
									for (var kk = 0; kk < this.listBoundRect.length; kk++) {
											_tmp = !this.intersectsRectRect(this.listBoundRect[kk].rect, this.list[i].rect);
											if (_tmp == false) {
													break;
											}
									}
							...
							if (_tmp === true && this.listBoundSegment.length > 0) {
									for (var kk = 0; kk < this.listBoundSegment.length; kk++) {
											_tmp = !this.intersectsSegmentRect(this.listBoundSegment[kk].segment, this.list[i].rect);
											if (_tmp == false) {
													break;
											}
									}
							...
							if (_tmp === true && this.listBoundCircle.length > 0) {
									for (var kk = 0; kk < this.listBoundCircle.length; kk++) {
											_tmp = !this.intersectsCircleRect(this.listBoundCircle[kk].circle, this.list[i].rect);
											if (_tmp == false) {
													break;
											}
									}
							...
							if (_tmp === true && good.length > 0) {
									for (var kk = 0; kk < good.length; kk++) {
											_tmp = !this.intersectsRectRect(this.list[good[kk]].rect, this.list[i].rect);
											if (_tmp == false) {
													break;
											}
									}
	...
	intersectsSegmentRect: function (segment, rect) {
			...
			return this.intersectsSegmentSegment(segment, {
					x1: rect.x,
					y1: rect.y,
					x2: rect.x + rect.w,
					y2: rect.y
	...
	intersectsPointRect: function (point, rect) {
			...
			return (point.x >= rect.x) && (point.y >= rect.y) && (point.y <= (rect.y + rect.h)) && (point.x <= (rect.x + rect.w));
	},
	intersectsSegmentSegment: function (segment1, segment2) {
			...
			if (Math.max(segment1.x1, segment1.x2) < Math.min(segment2.x1, segment2.x2)) return false;
			// ’ј 
			...
			if (
					(xa < Math.max(Math.min(segment1.x1, segment1.x2), Math.min(segment2.x1, segment2.x2))) ||
					(xa > Math.min(Math.max(segment1.x1, segment1.x2), Math.max(segment2.x1, segment2.x2)))
					) {
					return false;
			}
			return true;
